...
class DocumentService(CommonService):
    ...
    def get_filter_by_kb_id(cls, kb_id, keywords, run_status, types, suffix):
        ...
        total = query.count()
        grouped_rows = (
            query.select(cls.model.run, cls.model.suffix, fn.COUNT(cls.model.id).alias("count"))
            .group_by(cls.model.run, cls.model.suffix)
        )
        for row in grouped_rows:
            suffix_counter[row.suffix] = suffix_counter.get(row.suffix, 0) + row.count
            run_status_counter[str(row.run)] = run_status_counter.get(str(row.run), 0) + row.count

        metadata_facets = None
        if not any((keywords, run_status, types, suffix)):
            metadata_facets = DocMetadataService.get_metadata_filter_counts(kb_id)

        if metadata_facets is not None:
            metadata_counter, documents_with_metadata = metadata_facets
            empty_metadata_count = max(0, total - documents_with_metadata)
        else:
